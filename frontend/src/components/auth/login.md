// src/components/LoginForm.jsx (UPDATED for Stateless Token Auth)

import React, { useState } from "react";
// Import the custom Axios instance and the token setter function
import { useAuth } from '../../context/AuthContext'
import api, { setAuthToken } from "../../api/axios";
import { useNavigate } from "react-router-dom";

function LoginForm() {
  // ... state declarations (email, password, error, isLoggedIn) ...
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // No CSRF cookie step needed here!
      const response = await api.post("/login", {
        email,
        password,
      });

      // 2. Extract and save the token from the response
      const token = response.data.token;

      if (token) {
        // Use the setAuthToken utility to save token and set Axios header
        setAuthToken(token);
        setIsLoggedIn(true);
        login(email);
      ...
      // Handle 422 (Validation) or 401 (Invalid Credentials)
      const errorMessage =
  ...
  // ... rest of the component JSX (Tailwind styling) remains the same ...

  if (isLoggedIn) {
    return (
      // ... success message JSX ...
      <div className="min-h-screen flex items-center justify-center bg-gray-100">
        <div className="p-8 bg-green-100 text-green-800 border border-green-400 rounded-lg shadow-lg text-center max-w-sm w-full">
          <h2 className="text-2xl font-bold mb-4">🎉 Login Successful!</h2>
          <p>Token saved. You can now access protected routes.</p>
        </div>
      </div>
    );
  }
